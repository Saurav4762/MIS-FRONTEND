...
import { getOptionItemById, getOptionItemsByOptionListId } from "./option-item.api";
...
export const useOptionItemsByOptionList = (
	...
	return useQuery<OptionItem[]>({
		queryKey: optionItemKeys.byOptionList(
			normalizedOptionListId,
		),
		queryFn: () =>
			getOptionItemsByOptionListId(normalizedOptionListId),
		enabled:
			normalizedOptionListId.length > 0 &&
			normalizedOptionListName.length > 0,
	});
};
