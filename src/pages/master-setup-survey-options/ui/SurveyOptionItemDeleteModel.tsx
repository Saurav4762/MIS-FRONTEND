...
export function SurveyOptionItemDeleteModal({
	...
	return (
		...
		>
			<div className="mt-6 rounded-[var(--mis-card-radius)] border border-[var(--mis-color-error-100)] bg-[var(--mis-color-error-50)] px-4 py-4">
				<p className="text-sm text-[var(--mis-color-error-600)]">
					<span className="font-bold">Item:</span>{" "}
					{optionItem?.labelEn ?? "No item selected"}
				</p>
				<p className="mt-1 text-xs text-[var(--mis-color-ink-600)]">
					{optionItem?.labelNe ?? "Select an option item before deleting."}
				</p>
			</div>

			{!canDelete ? (
				<p className="mt-4 rounded-[var(--mis-field-radius)] border border-[var(--mis-color-error-100)] bg-[var(--mis-color-error-50)] px-4 py-3 text-sm font-semibold text-[var(--mis-color-error-600)]">
					Select an option item before deleting.
				</p>
			) : null}

			<div className="mt-7 flex flex-wrap justify-end gap-3 border-t border-[var(--mis-color-ink-200)] pt-5">
				<Button
					type="button"
					variant="ghost"
					onClick={handleClose}
